...
export function SearchContextProvider({
    ...
    const [queryState, setQueryState] = useLocalStorage<{
        orderByItems: Parameters<
            typeof itemsInfiniteQueryOptions
        >[0]['orderBy'];
        orderByAlbums: Parameters<
            typeof albumsInfiniteQueryOptions
        >[0]['orderBy'];
        orderDirection: 'ASC' | 'DESC';
    ...
    if (
        ...
        (debouncedQuery.length - debouncedQuery.replace(/\\+$/, '').length) %
            2 ===
            1
    ) {
...
export function useSearchContext() {
    ...
    if (!context) {
        throw new Error(
            'useSeachContext must be used within a SearchContextProvider'
        );
    }
    return context;
}
